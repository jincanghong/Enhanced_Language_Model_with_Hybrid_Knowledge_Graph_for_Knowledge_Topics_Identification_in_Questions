...
# __time__   = '2019/7/21 17:16'

import time,os
import torch, torch.nn as nn
from pybert.callback.progressbar import ProgressBar
from pybert.utils.utils import restore_checkpoint, model_device
...
class Trainer(object):
    def __init__(self,train_configs):
        ...
        self.criterion_cpu = train_configs['criterion_cpu']
        ...
        self.embed = torch.nn.Embedding.from_pretrained(train_configs['embed']).to(self.device)

        # 重载模型，进行训练
        if train_configs['resume']:
            self.logger.info(f"\nLoading checkpoint: {train_configs['resume']}")
            resume_list = restore_checkpoint(resume_path =os.path.join(configs['output']['checkpoint_dir'],
                                                                       train_configs['resume']),
                                             model=self.model, optimizer=self.optimizer)
            best = resume_list[2]
            self.model = resume_list[0]
            self.optimizer = resume_list[1]
            self.start_epoch = resume_list[3]
            if configs['callbacks']['save_best_only'] and self.model_checkpoint:
    ...
    def _valid_epoch(self,data):
        ...
        with torch.no_grad():
            ...
            loss = self.criterion_cpu(target = self.targets, output=self.outputs)
    ...
    def _train_epoch(self,data):
        ...
        for step, batch in enumerate(data):
            ...
            logits = self.model(input_ids, segment_ids, input_mask, input_ent, ent_mask)
            # 计算batch loss
            loss = self.criterion(output=logits, target=label) + self.mseloss(logits.sigmoid(), label)
            if len(self.n_gpu) >= 2:
                loss = loss.mean()
    ...
    def train(self,train_data,valid_data):
        ...
        #     break
        # ***************************************************************
        for ind, epoch in enumerate(range(self.start_epoch,self.start_epoch+self.epochs)):
            print(f"\n--------------------Epoch {epoch}/{self.start_epoch + self.epochs}------------------------")
