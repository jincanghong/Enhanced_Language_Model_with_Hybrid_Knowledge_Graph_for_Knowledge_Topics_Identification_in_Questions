...
def restore_checkpoint(resume_path,model = None,optimizer = None):
    ...
    best = checkpoint['best'] if 'best' in checkpoint else None
    start_epoch = checkpoint['epoch'] + 1
    if model:
        model.load_state_dict(checkpoint['state_dict'])
        model_to_save = model.module if hasattr(model, 'module') else model
        torch.save(model_to_save, './model/my_model/pytorch_model.bin')
        print("模型输出完成", '*'*20)
    if optimizer:
...
def load_bert(model_path,model = None,optimizer = None):
    ...
    best = checkpoint['best'] if 'best ' in checkpoint else 1e10
    start_epoch = checkpoint['epoch'] + 1
    if model:
...
def text_write(filename,data, np=False):
    if not np:
        with open(filename, 'w') as fw:
            for q_id, sentence, target in tqdm(data, desc='write data to disk'):
                target = [str(x) for x in target]
                line = '\t'.join([q_id, sentence, ";" .join(target)])
                if len(line.split('\t')) != 3:
                    print(len(line.split('\t')))
                    continue
                fw.write(line + '\n')
    else:
        pd.DataFrame(data).to_csv(filename, index=None, header=None, sep='\t')

def text_read(filename, np=False):
    if not np:
        res = []
        with open(filename, 'r')as fr:
            for line in fr.readlines():
                q_id, sentence, target = line.split('\t')
                target = [int(x) for x in target.split(';')]
                res.append((q_id, sentence, target))
        print("{}:{}".format(filename, len(res)))
    else:
        res = pd.read_csv(filename, sep='\t', index=None, header=None).values
    return res