...
class AccuracyThresh(Metric):
    ...
    def value(self):
        ...
        # 主要看 1的是否命中了
        y_pred = (self.y_pred >= self.thresh).cpu().numpy()
        y_true_index = np.where(self.y_true.cpu().numpy()==1) # get index
        acc = y_pred[y_true_index].sum() / len(y_true_index[0])
        acc2 = np.mean(((self.y_pred >= self.thresh) == self.y_true.byte()).float().cpu().numpy(), axis=1).sum() / data_size
        return (acc +acc2) / 2
...
class MultiLabelReport(Metric):
    ...
    def value(self):
        ...
        for i, label in self.id2label.items():
            if len(np.unique(self.y_true[:,i])) == 1:  # bug in roc_auc_score
                acc = accuracy_score(self.y_true[:, i], np.rint(self.y_prob[:,i]))
                print(f"label:{label} - acc: {acc:.4f}")
            else:
