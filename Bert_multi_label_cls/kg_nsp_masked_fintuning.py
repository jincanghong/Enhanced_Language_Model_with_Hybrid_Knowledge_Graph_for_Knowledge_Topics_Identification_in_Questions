...
from math_bert.novelty_modeling import BertForPreTraining
...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        t1, t2, is_correlation = self.random_sent(item)

        id_t1, content_t1 = t1[0], t1[1]
        id_t2, content_t2 = t2[0], t2[1]
        # tokenize
        tokens_a = self.tokenizer.tokenize(content_t1)
        tokens_b = self.tokenizer.tokenize(content_t2)

        # combine to one sample
        cur_example = InputExample(guid=cur_id, tokens_a=tokens_a, tokens_b=tokens_b, tokens_a_id=id_t1, tokens_b_id=id_t2,
        ...
        cur_tensors = (torch.tensor(cur_features.input_ids),
                       ...
                       torch.tensor(cur_features.is_correlation),
                       )
    ...
    def random_sent(self, index):
        ...
        t1_id, t2_id = t1[0], t2[0]
        is_correlation = self.calculate_sim(t1_id, t2_id)
        assert len(t1) > 0 and len(t2) > 0
        return t1, t2, is_correlation
    ...
    def get_corpus_line(self, item):
        ...
        t1 = ""
        t2 = ""
        t2_item = random.randint(0, self.corpus_lines-5)
        assert item < self.corpus_lines
        assert t2_item < self.corpus_lines

        if self.on_memory:
            sample = self.sample_to_doc[item]
            sample_t2 = self.sample_to_doc[t2_item]
            t1 = self.all_docs[sample["doc_id"]][sample["line"]]
            t2 = self.all_docs[sample_t2["doc_id"]][sample_t2["line"]]
            return t1, t2
        assert t1 != "" and t2 != ""
        return t1, t2

    def calculate_sim(self, t1_id, t2_id, threshold=0.5, get_score=False):
        entity_list_0, entity_list_1 = self.id_match_entity[t1_id] if t1_id in self.id_match_entity else [], \
                                       self.id_match_entity[t2_id] if t2_id in self.id_match_entity else []
        if len(entity_list_0) == 0 or len(entity_list_1) == 0:
            return 0
        entity_list_0 = [list(self.G[self.entity2id[entity_0]]) for entity_0 in entity_list_0]
        entity_list_1 = [list(self.G[self.entity2id[entity_1]]) for entity_1 in entity_list_1]
        ...
        if not get_score:
            return 1 if jacarrd_score > threshold else 0
        else:
            return jacarrd_score

class InputExample(object):
    """A single training/test example for the language model."""

    def __init__(self, guid, tokens_a, tokens_b=None, lm_labels=None, tokens_a_id=None, tokens_b_id=None,
                 is_correlation=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, lm_label_ids, input_ent, ent_mask, t_id, is_correlation):
...
def convert_example_to_features(example, max_seq_length, tokenizer, id_match_entity, entity2id, t_id2int):
    ...
    a_id = example.tokens_a_id
    b_id = example.tokens_b_id
    ...
    tokens_a, t1_label = random_word(tokens_a, tokenizer, id_match_entity, a_id)
    if a_id in id_match_entity:
        for ent in id_match_entity[a_id]:
            if ent != "UNK" and ent in entity2id:
                input_ent.append(entity2id[ent])
                ent_mask.append(1)

    tokens_b, t2_label = random_word(tokens_b, tokenizer, id_match_entity, b_id)
    ...
    if example.guid < 5:
        ...
        logger.info("Is Correlation: %s " % (example.is_correlation))
    ...
    features = InputFeatures(input_ids=input_ids,
                             ...
                             ent_mask=ent_mask,
                             t_id=t_id2int[a_id],
                             is_correlation=example.is_correlation)
    return features
...
def main():
    ...
    '''
    novelty部分只增加了maskLM操作 ， kg_novelty 增加了MaskedML与kg在model的引入, Nsp+KG+Mask
    '''
    parser.add_argument("--output_dir",
                        default='/home/cdk/python_project/bert_multi_tag_cls/model/math_all_pretrain_model/kg_nsp_novelty_math_6w_save_pretrain_model_0724',
                        type=str,
                        required=False,
    ...
    model.train()
    sample4calculation = 50
    upper_sample_bound = args.train_batch_size // n_gpu
    for ind_epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        with tqdm(train_dataloader, desc="Iteration") as tqdm_batch:
            for step, batch in enumerate(tqdm_batch):
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_mask, segment_ids, lm_label_ids, \
                input_ent, ent_mask, t_id_array, sentence_ab_is_correlation = batch
                input_ent = embed(input_ent+1)  # -1 -> 0
                batch_correlation_label = []
                batch_random_tuple4cls = []
                if upper_sample_bound * 4 <= input_ids.shape[0]:
                    batch_random_tuple4cls = torch.unique(torch.randint(0, upper_sample_bound, (sample4calculation*100, 2)), dim=0)
                    ...
                    for tmp in batch_random_tuple4cls:
                        ...
                        jacarrd_score = train_dataset.calculate_sim(t_id_0, t_id_1, threshold=0.5, get_score=True)
                        batch_correlation_label.append(jacarrd_score * 10)

                    batch_correlation_label = torch.Tensor(batch_correlation_label ).to(device)
                ...
                loss = model(input_ids, input_ent, ent_mask, segment_ids, input_mask, lm_label_ids,
                             batch_random_tuple4cls, batch_correlation_label, sentence_ab_is_correlation)
                if n_gpu > 1:
