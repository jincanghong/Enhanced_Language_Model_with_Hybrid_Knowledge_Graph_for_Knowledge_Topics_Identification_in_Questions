...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, lm_label_ids, is_correlation):
...
def convert_example_to_features(example, max_seq_length, tokenizer, id_match_entity):
    """
    ...
    for ind_epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        with tqdm(train_dataloader, desc="Iteration") as tqdm_batch:
            for step, batch in enumerate(tqdm_batch): # batch的问题
                ...
                input_ids, input_mask, segment_ids, lm_label_ids, sentence_ab_is_correlation = batch
                loss = model(input_ids, segment_ids, input_mask, lm_label_ids, sentence_ab_is_correlation)
                if n_gpu > 1:
